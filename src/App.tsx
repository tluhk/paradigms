...
import { concepts as englishConcepts } from './content/foundations';
...
export default function App() {
  ...
  const concepts = language === 'et' ? estonianConcepts : englishConcepts;
  ...
  return <>
    ...
    <main id="main">
      {screen === 'home' ? <>
        ...
        <section className="deck-layout" aria-label={t("Foundations deck")}>
          <div className="deck-art" aria-hidden="true"><span className="art-note">{t("THE BUILDING BLOCKS OF RESEARCH")}</span><div className="paper paper-back"></div><div className="paper paper-middle"></div><div className="paper paper-front"><span className="paper-number">01 / 06 <span>✳</span></span><div className="orbit"><span></span></div><div><span className="eyebrow">{t("THE NATURE OF REALITY")}</span><h2>{t("Ontology")}</h2><p>{t("What is reality like?")}</p></div><span className="paper-footer">{t("FOUNDATIONS")} <span>↗</span></span></div><span className="art-bottom">{t("A small deck. A clearer perspective.")}</span></div>
          <div className="deck-details"><div className="deck-label"><span className="pill">{t("STARTER DECK")}</span><span>{t("6 concepts · At your own pace")}</span></div><h2>{t("Foundations of research")}</h2><p>{t("Reality, knowledge, values, and the choices behind a study. Get to know the ideas that connect them.")}</p><div className="concept-tags">{concepts.map(c => <span key={c.id}>{c.term}</span>)}</div><div className="actions"><button className="primary" onClick={() => learn()}>{t("Explore the cards")} <span>↗</span></button><button className="secondary" onClick={() => start()}>{t("Practise matching")} <span>→</span></button></div><button className="secondary placement-entry" onClick={() => setScreen('placement')}>{t("Place the cards")} <span>↗</span></button><p className="small-note">{t("Start with learning, or jump straight into practice.")}</p></div>
        </section>
        ...
        {screen === 'placement' && <Placement concepts={concepts} language={language} onCheck={answers => {
