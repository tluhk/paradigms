  ...
  return <>
    ...
    <main id="main">
      {screen === 'home' ? <>
        ...
        {screen === 'learn' && <><div className="play-top"><span className="eyebrow">{t("FOUNDATIONS · LEARN")}</span><span>{language === 'et' ? `Kaart ${cardIndex + 1} / 6` : `Card ${cardIndex + 1} of 6`}</span></div><article className="learning-flip" key={card.id}>
          <div className={`learning-flip-inner ${revealed ? 'is-flipped' : ''}`}>
            <div className="learning-card learning-face learning-front" aria-hidden={revealed} inert={revealed}>
              <span className="pill">{card.cue}</span><h1 ref={!revealed ? heading : undefined} tabIndex={-1}>{card.term}</h1><p className="guiding">{card.question}</p>
            ...
            <div className="learning-card learning-face learning-back" aria-hidden={!revealed} inert={!revealed}>
              <span className="pill">{card.cue}</span><h1 ref={revealed ? heading : undefined} tabIndex={-1}>{card.term}</h1>
