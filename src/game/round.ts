...
export function createRound(ids = concepts.map(c => c.id), initialScore: number | null = null): Round {
  return { questions: shuffle(ids).map(id => ({ id, options: shuffle([id, ...shuffle(concepts.filter(c => c.id !== id).map(c => c.id)).slice(0, 2)]) })), index: 0, initialScore, initialTotal: concepts.length };
}
