import EvidencePractice from './EvidencePractice';
import { evidenceExercises } from './content/evidence';
...
export default function StudyBuilder({ language, sessions: suppliedSessions, onStorageError }: { language: Language; sessions?: SessionStore; onStorageError?: () => void }) {
  ...
  function reset(next = index, fresh = true) {
    ...
    if (fresh) sessions.remove(`evidence:${studies[next].id}`);
  ...
  return <div className="study-builder">
    ...
    {checked && evidenceExercises[study.id] && <EvidencePractice key={study.id} exercise={evidenceExercises[study.id]} language={language} sessions={sessions} onStorageError={onStorageError} />}
